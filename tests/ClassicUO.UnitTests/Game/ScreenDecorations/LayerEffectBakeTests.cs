...
namespace ClassicUO.UnitTests.Game.ScreenDecorations
{
    ...
    public class LayerEffectBakeTests
    {
        ...
        public void RecoveringAndRebakingALayerChangesNothing(ScreenOverlayPreset preset)
        {
            foreach (OverlayLayer layer in BakePreset(preset))
            {
        ...
        [Theory]
        [MemberData(nameof(ShippedLooks))]
        public void EveryBuiltInProfileBakesExactlyLikeItsPreset(Guid profileId, ScreenOverlayPreset preset)
        {
            EffectProfile profile = BuiltInProfiles.Find(profileId);

            profile.Should().NotBeNull();
            BakeProfile(profile).Should().BeEquivalentTo(BakePreset(preset));
        }
        ...
        {
            foreach (object[] look in ShippedLooks())
                yield return [look[1]];
        }

        /// <summary>Every shipped look, paired with the preset that authors it.</summary>
        public static IEnumerable<object[]> ShippedLooks() =>
            new[]
            {
