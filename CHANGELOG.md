...
### Fixes
* Fixed the bandage agent monopolizing the shared action queue: heals still run through the queue but no longer reset the global action cooldown (they are throttled by the agent's own timer), and the queued heal is re-validated when it runs so it no longer fires - or resets the cooldown - on a round where no heal is actually needed - [P.R 846](https://github.com/PlayTazUO/TazUO/pull/846) ([bittiez](https://github.com/bittiez))
