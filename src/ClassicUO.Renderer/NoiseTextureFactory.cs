...
namespace ClassicUO.Renderer
{
    ...
    public static class PeriodicNoise
    {
        ...
        private static float GradDot(int latticeX, int latticeY, int period, int seed, float dx, float dy)
        {
            ...
            Vector2 g = Gradients[h % Gradients.Length];
        ...
        public static float Worley(float u, float v, int period, int seed)
        {
            ...
            for (int oy = -1; oy <= 1; oy++)
            {
                for (int ox = -1; ox <= 1; ox++)
                {
                    int cellX = cx + ox;
                    int cellY = cy + oy;
                    Vector2 fp = CellFeaturePoint(FloorMod(cellX, period), FloorMod(cellY, period), period, seed);
    ...
    /// <summary>
    /// Builds the single tiling noise texture consumed by <see cref="ScreenOverlayEffect"/>. One
    ...
    public static class NoiseTextureFactory
    {
        ...
        private const int GasBasePeriod = 4;
        private const int DetailBasePeriod = 8;
        private const int RidgeBasePeriod = 4;
        private const int WorleyPeriod = 8;
        ...
        public static Texture2D Create(GraphicsDevice device)
        {
            ...
            for (int y = 0; y < TextureSize; y++)
            {
                ...
                for (int x = 0; x < TextureSize; x++)
                {
                    ...
                    float r = PeriodicNoise.Fbm(u, v, GasBasePeriod, Octaves, Seed);
                    float g = PeriodicNoise.Fbm(u, v, DetailBasePeriod, Octaves, Seed + 1000);

                    float ridgeFbm = PeriodicNoise.Fbm(u, v, RidgeBasePeriod, Octaves, Seed + 2000);
                    ...
                    float a = 1f - PeriodicNoise.Worley(u, v, WorleyPeriod, Seed + 3000);
