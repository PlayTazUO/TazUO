...
internal static class OverlayProfilesTab
{
    ...
    /// <summary>
    /// A new look starts from a single flat tint rather than from nothing: an empty stack draws
    /// nothing at all, which reads as the editor being broken rather than as a blank canvas.
    /// </summary>
    ...
    private static EffectProfile Create(OverlaySystemSettings overlays, string name)
    {
        var created = new EffectProfile
        {
            Name = name,
